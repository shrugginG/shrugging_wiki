...
- Don’t use spaces around the
  ...
  sign when used to indicate a keyword argument, or when used to indicate a default value for an
...
- PEP 257
  ...
  describes good docstring conventions. Note that most importantly, the
...
- ```
  ...
  (or CapWords, or CamelCase – so named because of the bumpy look of its letters
...
- Users who don’t want to use type checkers are free to ignore them. However, it is expected that users of third party library packages may want to run type checkers over those packages. For this purpose [PEP 484](https://peps.python.org/pep-0484) recommends the use of stub files: .pyi files that are read by the type checker in preference of the corresponding .py files. Stub files can be distributed with a library, or separately (with the library author’s permission) through the typeshed repo [[5\]](https://peps.python.org/pep-0008/#id9).
...
Last modified: [2022-01-25 15:20:55 GMT](https://github.com/python/peps/commits/main/pep-0008.txt)